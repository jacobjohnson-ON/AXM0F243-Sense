...
#ifdef BMA400
uint8_t buffer_txBMA[13] = {0};
float x = 0, y= 0, z = 0;
bool intBMA = 0;
#endif// BMA400

#ifdef BME680
uint8_t buffer_txBME[13] = {0};
...
static void wakeup_callback(struct wtimer_desc __xdata *desc) // read and send BME680 data
{
    desc;
        ...
        t = data.temperature / 100.0;
        p = data.pressure / 100.0;
        h = data.humidity / 1000.0;
        memcpy(&buffer_txBME[1],&t,sizeof(t)); // 32 bits float
        memcpy(&buffer_txBME[5],&p,sizeof(p)); // 32 bits float
        memcpy(&buffer_txBME[9],&h,sizeof(h)); // 32 bits float
        transmit_packet(buffer_txBME, sizeof(buffer_txBME));
//        transmit_packet(test_packet,sizeof(test_packet));
    #endif
    #ifdef BMA400
