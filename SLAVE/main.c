    ...
    switch (st->status)
    {
        case AXRADIO_STAT_RECEIVE:
            ...
            #else  // RADIO_MODE
            {
                ...
                if(st->error == AXRADIO_ERR_NOERROR)
                {
                    addr = st->u.rx.pktdata;
                    #ifdef BME680
                    if((st->u.rx.pktdata[4] == (uint8_t)'E'))
                    {
                    ...
                    #endif
                    #ifdef BMA400
                    if((st->u.rx.pktdata[4] == (uint8_t)'A'))
                    {
//                        x = *(float *)&(st->u.rx.pktdata[5]);
//                        y = *(float *)&(st->u.rx.pktdata[9]);
//                        z = *(float *)&(st->u.rx.pktdata[13]);
//                        sprintf(txBuffer,"DEVICE NUMBER %d ; X: %.2f , Y: %.2f , Z : %.2f \r",addr[3],x,y,z);
